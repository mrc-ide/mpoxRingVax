...
set_rdhs_config(email = "charles.whittaker16@imperial.ac.uk",
                project = "Household Heterogeneity in Malaria and Febrile Status")
...
south_kivu$hh_occupations <- vector("list", nrow(south_kivu))
south_kivu$hh_ages_grouped <- vector("list", nrow(south_kivu))

for (i in 1:nrow(south_kivu)) {
  temp_ages <- unlist(south_kivu$hh_ages[i][[1]])
  south_kivu$hh_occupations[i] <- list(rep("genPop", length(temp_ages)))
  south_kivu$hh_ages_grouped[i] <- list(ifelse(temp_ages <= 5, "0-5", ifelse(temp_ages <= 18, "5-18", "18+")))
}
