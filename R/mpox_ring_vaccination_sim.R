...
mpox_ring_vaccination_sim <- function(## Sexual Transmission Parameters
  ...
  # During each loop of the "while" loop below, we select an infection for whom we have not yet generated both SECONDARY and TERTIARY
  # offspring for.
  ...
  # quarantining and ring-vaccination) and again prune the transmission tree as relevant. Remember, we use INDEX, SECONDARY and
  # TERTIARY relationally, i.e. the TERTIARY offspring of a particular INDEX infection will ALSO be the SECONDARY offspring of that
  # INDEX infection's SECONDARY offspring.
  #
  # What this means is that when we select a new INDEX infection for whom we have not yet generated both SECONDARY and TERTIARY
  # offspring for, we have usually already generated their SECONDARY offspring (when they were a SECONDARY offspring themself for
  # a prior INDEX infection). If that is the case, and we've already generated the SECONDARY offspring for this new INDEX infection
  # (when they were a SECONDARY infection to a preceding INDEX infection), we skip the steps above (which were implemented before)
  # and instead we simulate a potential "2nd chance" at ring vaccination these infections get. All infections in this simulation
  ...
  we skip the step directly below (which is about generating
  ...
  while ((any(is.na(tdf$n_offspring)) | any(tdf$n_offspring_post_pruning != 0 & tdf$secondary_offspring_generated == FALSE)) & nrow(tdf) <= check_final_size) {
    ...
    if (index_secondary_offspring_generated == FALSE) {
      ...
      #   CFWNote: Need to chec, when vaccine_efficacy_transmission is 1, does that introduce NAs that get passed through to next section
      ####################################################################################################################################
      if (index_vaccinated == 1) { # Only do this if index infection was vaccinated
        ...
        if (index_time_protected < index_time_infection) {
          ...
          index_offspring_vaccine_averted_transmission_route <- index_offspring_function_draw$offspring_characteristics$transmission_route[index_offspring_vaccine_averted_index]
          index_offspring_vaccine_averted_hh_member_index <- index_offspring_function_draw$offspring_characteristics$hh_member_index[index_offspring_vaccine_averted_index]
          ...
          if ("household" %in% index_offspring_vaccine_averted_transmission_route) {
