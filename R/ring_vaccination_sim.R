  ...
  while ((any(is.na(tdf$n_offspring)) | any(tdf$n_offspring_post_pruning != 0 & tdf$secondary_offspring_generated == FALSE)) & nrow(tdf) <= check_final_size) {
    ...
    if (index_secondary_offspring_generated == FALSE) {
      ...
      if (index_quarantine == 1 & index_asymptomatic == 0 & index_n_offspring != 0) {
        ...
        index_quarantine_outcome <- implement_quarantine(symptom_onset_time = index_onset_time,
                                                         quarantine_time = index_quarantine_time,
                                                         n_offspring = index_n_offspring,
        ...
        index_n_offspring <- index_quarantine_outcome$updated_n_offspring
        secondary_infection_times <- index_quarantine_outcome$updated_infection_times
        index_offspring_function_draw <- index_quarantine_outcome$updated_offspring_function_draw
    ...
    if (index_pruned_n_offspring != 0) {
      ...
      for (i in 1:index_pruned_n_offspring) {
        ...
        if (secondary_quarantine == 1 & secondary_asymptomatic == 0 & secondary_n_offspring != 0) {
          ...
          # Implement quarantining for index infection
          secondary_quarantine_outcome <- implement_quarantine(symptom_onset_time = secondary_onset_time,
                                                               quarantine_time = secondary_time_quarantine,
                                                               n_offspring = secondary_n_offspring,
          ...
          # Updating number offspring, their infection times and characteristics to reflect removals due to quarantining
          secondary_n_offspring <- secondary_quarantine_outcome$updated_n_offspring
          tertiary_infection_times <- secondary_quarantine_outcome$updated_infection_times
          secondary_offspring_function_draw <- secondary_quarantine_outcome$updated_offspring_function_draw
